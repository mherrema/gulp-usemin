module.exports = function(file, blocks, options, push, callback) {
  ...
  var jsCounter = 0;
  var cssCounter = 0;
  var htmlPathPrefix = options ? options.htmlPathPrefix : null;
      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            html[i] += '<script src="' + htmlPathPrefix + name.replace(path.basename(name), path.basename(file.path)) +  '"' + createHTMLAttributes(jsAttributes, jsCounter++) +'></script>';
          resolve();
