      ...
      if (typeof block == 'string') {
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.js')
            html[i] += '<script src="' + htmlPathPrefix + name.replace(path.basename(name), path.basename(file.path)) +  '"' + createHTMLAttributes(jsAttributes, jsCounter++)+ htmlPathSuffix  +'></script>';
          resolve();
        ...
        pipeline(block.name, block.files, block.tasks, function(name, file) {
          ...
          if (path.extname(file.path) == '.css') {
            html[i] += '<link rel="stylesheet" href="' + htmlPathPrefix + name.replace(path.basename(name), path.basename(file.path))
              ...
              + createHTMLAttributes(cssAttributes, cssCounter++)+ htmlPathSuffix  +'/>';
          }
          resolve();
        }.bind(this, block.nameInHTML));
      }
